...
export default function StepScore({ name, score, setScore, onBack, onNext }: StepScoreProps) {
    ...
    const handleScoreSubmit = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const { valid, score: validatedScore, message } = validateScoreInput(value);
    ...
    return (
        <div>
            ...
            <label htmlFor="stepscore-score" className="block text-sm font-medium text-gray-700 mb-2">
                Score
            </label>
            <input
                type="text"
                id="stepscore-score"
                name="stepscore-score"
                value={score ?? ""}
                onChange={handleScoreSubmit}
                className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm 
    focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-lg"
            ...
            <div className="flex justify-between mt-6 space-x-4">
                ...
                <button
                    onClick={handleNext}
                    className={`
                        flex-1 py-2 px-4 border border-transparent rounded-md shadow-sm 
