...
const StepContainer: React.FC<stepContainerProps> = () => {
    ...
    const handleUpdateScoreOnly = async (_: string, newScore: number) => {
        try {
            ...
            Swal.fire({
                icon: "error",
                text: error.message || "Error updating score",
            });
        }
    };
    ...
    const renderStepContent = () => {
        const gpxTitle = (
            <h1 className="text-xl font-bold text-gray-800 mb-6 text-center">Grade Point Average ( GPX )</h1>
        );
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-100 to-purple-200">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-sm mx-auto">{renderStepContent()}</div>
        </div>
    );
};
